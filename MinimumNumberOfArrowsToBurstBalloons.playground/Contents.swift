/**
 There are a number of spherical balloons spread in two-dimensional space.
 For each balloon, provided input is the start and end coordinates of the horizontal
 diameter. Since it's horizontal, y-coordinates don't matter and hence the
 x-coordinates of start and end of the diameter suffice. Start is always smaller
 an end. There will be at most 104 balloons.

 An arrow can be shot up exactly vertically from different points along the x-axis.
 A balloon with xstart and xend bursts by an arrow shot at x if xstart ≤ x ≤ xend.
 There is no limit to the number of arrows that can be shot. An arrow once shot keeps
 travelling up infinitely. The problem is to find the minimum number of arrows that
 must be shot to burst all balloons.

 Input:
 [[10,16], [2,8], [1,6], [7,12]]
 Output: 2

 Explanation:
 One way is to shoot one arrow for example at x = 6 (bursting the balloons [2,8]
 and [1,6]) and another arrow at x = 11 (bursting the other two balloons).
 */
class Solution {
    /**
     Sort the input by ending time. Keep track of the end. Once we hit a balloon
     that starts AFTER the latest ending coord that we've been tracking, we know there is
     no overlap, thus incrementing the count of arrows required.
     - Complexity: O(n log n) for sorting.
     */
    func findMinArrowShots(_ points: [[Int]]) -> Int {
        guard points.count > 0 else { return 0 }

        let points = points.sorted { $0[1] < $1[1] }
        var arrows = 1
        var prevEnd = points[0][1]
        var currStart = 0, currEnd = 0

        for p in points {
            currStart = p[0]
            currEnd = p[1]

            // We've hit a balloon that is not overlapping with the previous one.
            // Increment and compare it to this balloon's end!
            if currStart > prevEnd {
                arrows += 1
                prevEnd = currEnd
            }
        }

        return arrows
    }
}

print(Solution().findMinArrowShots([[10,16], [2,8], [1,6], [7,12]]))
